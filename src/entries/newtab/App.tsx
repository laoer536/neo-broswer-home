...
export default function NewTabApp() {
  ...
  const handleAddCountdown = useCallback((event: CountdownEvent) => {
  ...
  return (
    <div className="relative w-screen h-screen overflow-hidden" style={bgStyle}>
      ...
      {/* 主内容区 — 搜索框独立绝对居中 */}
      <div className="relative z-10 w-full max-w-2xl px-8" style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)' }}>
        <SearchBar
          searchEngine={data.searchEngine}
          onEngineChange={engine => setData({ searchEngine: engine })}
        />
      </div>
