...
function useDaysLeft(dateStr: string): UseDaysLeftResult {
  ...
  return { days, isPast, isToday, daysLabel, colorClass }
}

function CountdownCard(props: CountdownCardProps) {
  const { event, onDelete } = props
  const { daysLabel, colorClass } = useDaysLeft(event.date)
  ...
  return (
    <div className="group relative bg-white/10 backdrop-blur-sm border border-white/15 rounded-xl px-4 py-3 flex items-center gap-3 hover:bg-white/15 transition-all">
      ...
      <div className={`text-sm font-semibold shrink-0 ${colorClass}`}>{daysLabel}</div>
      <button
        onClick={onDelete}
        className="opacity-0 group-hover:opacity-100 absolute -top-2 -right-2 w-5 h-5 bg-red-500 rounded-full flex items-center justify-center transition-all duration-200 hover:bg-red-600 hover:scale-110 cursor-pointer"
      >
        <svg
          className="w-3 h-3 text-white block relative -top-px"
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
        >
...
export function CountdownPanel(props: CountdownPanelProps) {
  const { events, onAdd, onDelete } = props
  ...
  return (
    <div className="w-full max-w-sm">
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        >
          <svg
            className={`w-3.5 h-3.5 text-white transition-transform ${showForm ? 'rotate-45' : ''}`}
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
          >
      ...
      <div className="space-y-2">
        {sorted.length === 0 ? (
          <div className="text-white/30 text-sm text-center py-4">还没有倒计时，点击 + 添加</div>
        ) : (
          sorted.map((event) => <CountdownCard key={event.id} event={event} onDelete={() => onDelete(event.id)} />)
        )}
      </div>
    </div>
  )
