...
import { SEARCH_ENGINES, SEARCH_ENGINE_LABELS } from '@/types'
...
export function SearchBar(props: SearchBarProps) {
  const { searchEngine, onEngineChange } = props
      ...
      <div className="flex items-center bg-white/15 backdrop-blur-md border border-white/25 rounded-2xl overflow-visible shadow-2xl transition-all focus-within:bg-white/20 focus-within:border-white/40">
        ...
        <div ref={dropdownRef} className="relative">
          ...
          >
            ...
            <svg
              className={`w-3.5 h-3.5 transition-transform ${showEngines ? 'rotate-180' : ''}`}
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
        ...
        {/* 搜索按钮 */}
        <button onClick={handleSearch} className="px-4 py-3.5 text-white/60 hover:text-white transition-colors">
