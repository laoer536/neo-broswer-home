...
interface SettingsPanelProps {
  ...
  onSetSearchEngine: (engine: AppData['searchEngine']) => void
  onExport: () => void
  onImport: (file: File) => Promise<void>
}

const PRESET_COLORS = [
  '#0f172a', '#1e1b4b', '#064e3b', '#1c1917',
  '#0c1a2e', '#1a0533', '#0a1628', '#18181b',
]
        ...
        <div className="p-6 space-y-6">
                ...
                <>
                  ...
                  {data.backgroundImage && (
                    <div className="relative rounded-xl overflow-hidden h-24">
                      ...
                      />
                      <button
                        onClick={() => { onSetBackground(null); onSetBackgroundType('color') }}
          ...
          <section>
            ...
            <div className="grid grid-cols-2 gap-2">
              {(['bing', 'google', 'baidu', 'duckduckgo'] as AppData['searchEngine'][]).map(engine => {
                const labels: Record<string, string> = { bing: 'Bing', google: 'Google', baidu: '百度', duckduckgo: 'DuckDuckGo' }
                return (
                  <button
                    key={engine}
          ...
          <section>
            ...
            <div className="space-y-2">
              ...
              {importMsg && (
                <p className="text-xs text-white/60 text-center">{importMsg}</p>
              )}
            </div>
          </section>

          {/* 版本信息 */}
          <div className="text-center text-white/20 text-xs pt-4 border-t border-white/10">
            Neo Browser Home v1.0.0
          </div>
        </div>
      </div>
    </div>
